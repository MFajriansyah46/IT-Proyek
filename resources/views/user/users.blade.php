        ...
        @foreach ($users as $i => $user)
            <tr class="hover:bg-yellow-200">
                ...
                <x-table.data>
                    ...
                    @else
                        <svg class="h-8 w-8 rounded-full ml-6" xmlns="http://www.w3.org/2000/svg" width="1.6rem" height="1.6rem" viewBox="0 0 24 24">
                            <path fill="#787878" d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10s10-4.48 10-10S17.52 2 12 2m0 4c1.93 0 3.5 1.57 3.5 3.5S13.93 13 12 13s-3.5-1.57-3.5-3.5S10.07 6 12 6m0 14c-2.03 0-4.43-.82-6.14-2.88a9.95 9.95 0 0 1 12.28 0C16.43 19.18 14.03 20 12 20" />
                        </svg>
                    @endif
                </x-table.data>
                </div>
                <x-table.data>
                    <div class="flex gap-1 my-1">
                        ...
                        <form method="post" action="/users/delete" class="user-delete-form" data-user-id="{{ $user->id }}">
                            @csrf
                            <input type="hidden" name="remember_token" value="{{ $user->remember_token }}">

                            <button type="button" class="delete-user-button" data-user-id="{{ $user->id }}">
