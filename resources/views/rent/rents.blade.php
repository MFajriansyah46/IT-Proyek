<x-layout>
    ...
    <x-table.header :headers="['Tenant', 'Room', 'Ends At','']">
        ...
        <tr class="hover:bg-yellow-200">
                    ...
                    $(document).ready(function() {
                        ...
                        const countdownInterval = setInterval(function() {
                            ...
                            if (distance < 0) {
                                ...
                                window.location.href='/timeout/{{ $rent->id }}';
                            }
                        }, 1000);
                    });
                </script>
            ...
            <x-table.data>
                <form action="/timeout/{{ $rent->id }}" class="rent-discard-form" id="form-delete-{{ $rent->id }}" data-rent-id="{{ $rent->id }}">
                    @csrf
                    <button type="button" class="discard-rent-button" data-room-id="{{ $rent->id }}">
                        <div class="p-1 bg-red-500 hover:bg-red-400 font-medium text-white text-sm rounded-lg w-20 text-center cursor-pointer">Discard</div>
                    </button>
                </form>
            </x-table.data>
        </tr>
