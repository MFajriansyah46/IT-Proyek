<x-layout>
    <x-slot:title>{{ $title }}</x-slot>
        <a href="#">
            <button type="button"
                class="flex mb-2 ml-auto px-4 py-2 text-sm font-medium text-white bg-yellow-300 rounded-sm hover:bg-yellow-400 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-500 dark:hover:bg-blue-600 dark:focus:ring-blue-800">
        ...
        <x-table.header :headers="['Unit', 'Alamat', 'Latitude', 'Longtitude','Aksi']">
            <?php $i = 1; ?>
            @foreach ($buildings as $building)
                <tr class="hover:bg-yellow-200">
                    <x-table.data><?php echo $i; $i++; ?></x-table.data>
                    ...
                    <x-table.data>{{ $building->longtitude }}</x-table.data>
                    <x-table.data>
