<x-layout>
    <div class="px-8 bg-white shadow-md max-w-3xl ml-auto mr-auto mt-4 rounded-md"><br>
        ...
        <h1 class="text-5xl font-bold text-center text-gray-800 mt-4">Edit Room {{ $room->room_number }}</h1>
        <form action="/rooms/update/{{ $room->room_id }}" method="post" class="mb-20 mx-auto max-w-lg sm:max-w-xl md:max-w-2xl lg:max-w-3xl flex flex-col bg-white shadow-md rounded-lg p-6" onsubmit="return validateForm()" enctype="multipart/form-data">
            @csrf
            ...
            <div class="container mt-5 mb-5">
                <div class="flex justify-center">
                    <label for="upload-room-Image" class="border-2 border-dashed border-gray-300 p-10 w-full max-w-lg text-center cursor-pointer" id="drop-area">
                        ...
                        <img id="preview-room-image" src="{{ asset('storage/' . $room->images) }}" alt="Gambar Kamar" class="hidden w-full h-auto">
                        <input type="file" id="upload-room-Image" name="images" accept="image/*" class="hidden" onchange="previewImage(event)">
                        </label>
                </div>
            </div>
            ...
            <input type="number" name="price" class="border border-gray-300 rounded-md p-2 mb-6" value="{{ old('price', $room->price) }}" required>

            <label class="text-lg font-semibold mb-2">Kecepatan Internet</label>
            <input type="text" name="internet_speed" class="border border-gray-300 rounded-md p-2 mb-6" value="{{ old('internet_speed', $room->internet_speed) }}" required>
            ...
            <button type="submit" class="mt-4 bg-blue-600 text-white font-semibold py-2 px-4 rounded-md hover:bg-blue-700 focus:outline-none">Update</button>
