<x-layout>
    <div class="px-8 bg-white shadow-md max-w-3xl ml-auto mr-auto mt-4 rounded-md"><br>
        ...
        <form action="/rooms/update/{{ $room->id_kamar }}" method="post" class="mb-20 mx-auto max-w-lg sm:max-w-xl md:max-w-2xl lg:max-w-3xl flex flex-col bg-white shadow-md rounded-lg p-6" onsubmit="return validateForm()" enctype="multipart/form-data">
            @csrf
            ...
            <div class="container mt-5 mb-5">
                <div class="flex justify-center">
                    <label for="upload-room-Image" class="border-2 border-dashed border-gray-300 p-10 w-full max-w-lg text-center cursor-pointer" id="drop-area">
                        ...
                        <img id="preview-room-image" src="{{ asset('storage/' . $room->gambar_kamar) }}" alt="Gambar Kamar" class="hidden w-full h-auto">
            ...
            <input type="number" name="no_kamar" class="border border-gray-300 rounded-md p-2 mb-6" value="{{ $room->no_kamar }}" required>

            <label class="text-lg font-semibold mb-2">Unit Bangunan</label>
            <select name="id_bangunan" class="border border-gray-300 rounded-md p-2 mb-6" required>
                @foreach(auth()->user()->buildings as $building)
                    <option value="{{ $building->id_bangunan }}" {{ $building->id_bangunan == $room->id_bangunan ? 'selected' : '' }}>
                        {{ $building->unit_bangunan }} - {{ $building->alamat_bangunan }}
                    </option>
                @endforeach
            </select>
