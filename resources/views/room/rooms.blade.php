    ...
    <x-table.header :headers="['No Kamar', 'Harga Kamar', 'Kecepatan Internet', 'Rating Kamar', 'Aksi']">
        @foreach ($rooms as $i=>$room)
            <tr class="hover:bg-yellow-200">
                ...
                <x-table.data>{{ $rooms->no_kamar }}</x-table.data>
                <x-table.data>{{ $rooms->harga_kamar }}</x-table.data>
                <x-table.data>{{ $rooms->kecepatan_internet }}</x-table.data>
                <x-table.data>{{ $rooms->rating_kamar }}</x-table.data>
                <x-table.data>
